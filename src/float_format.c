                                ...
                                float_parts_t *dec)
{
    ...
    uint64_t exponent = 0;
    int magnitude = 0;
    p = parse_big_unsigned(p, end, &start, &exponent, &magnitude);
    if (!p)
        return NULL;
    if (!dec->significand) {
    ...
    if (exp_sign) {
        if (p - start != magnitude) { /* underflow? */
            dec->type = BINARY64_TYPE_ZERO;
            return p;
        }
...
ssize_t binary64_from_string(const char *buffer, size_t size, uint64_t *value)
{
    ...
    const char *p = parse_float(buffer, end, &dec);
    if (!p) {
        errno = EILSEQ;
        return -1;
    }
